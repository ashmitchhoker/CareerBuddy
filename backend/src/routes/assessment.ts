...
router.post('/start', authenticateToken, async (req: Request, res: Response) => {
  try {
    ...
    const nextAssessmentNumber = maxAssessment ? maxAssessment.assessment_number + 1 : 1;
...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (!['aptitude', 'values', 'personal'].includes(testType)) {
...
router.post('/submit', authenticateToken, async (req: Request, res: Response) => {
  try {
    ...
    const testOrder = ['aptitude', 'values', 'personal'];
...
router.post('/complete', authenticateToken, async (req: Request, res: Response) => {
  try {
    ...
    // Validate payload
    if (!payload.userProfile || !payload.tests || !Array.isArray(payload.tests)) {
    ...
    console.log(`Processing assessment for user: ${payload.userProfile.name}`);
    ...
    try {
      ...
      // Build assessment summary for chatbot
      ...
      if (!existingInitialMessage) {
        ...
        const userLanguage = (payload.userProfile.language || 'en') as 'en' | 'hi' | 'te' | 'ta' | 'bn' | 'gu';
