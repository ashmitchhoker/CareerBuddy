...
//   /**
//    * Run Python script with commands and get responses
//    */
//   private async runPythonScript(commands: any[]): Promise<any[]> {
//     return new Promise((resolve, reject) => {
//       try {
//         const pythonScriptPath = path.join(__dirname, 'rag_service.py');
//         const backendDir = path.resolve(__dirname, '../..');
...
//         console.log(`[RAG] Running Python script: ${pythonExecutable}`);
...
//           const msg = data.toString().trim();
//           if (!msg.includes('redirects.py') && !msg.includes('Loaded .env')) {
//             console.error(`[RAG]: ${msg}`);
//           }
//         });
...
//           if (code !== 0) {
//             console.error('[RAG] Python process exited with code:', code);
//             console.error('[RAG] Error output:', errorOutput);
//             reject(new Error(`RAG service failed (exit code: ${code})`));
//             return;
//           }
...
//             resolve(responses);
//           } catch (error: any) {
//             console.error('[RAG] Failed to parse responses:', output);
//             reject(new Error(`Failed to parse RAG responses: ${error.message}`));
//           }
//         });
...
//         const input = commands.map(cmd => JSON.stringify(cmd)).join('\n') + '\n';
...
//         setTimeout(() => {
//           pythonProcess.kill();
//           reject(new Error('RAG service timeout (60s)'));
//         }, 60000);

//       } catch (error: any) {
//         reject(new Error(`RAG error: ${error.message}`));
//       }
//     });
//   }
...
//     const responses = await this.runPythonScript(commands);
    
//     // Second response should be the chat response
//     const chatResponse = responses[1];
//     if (chatResponse && chatResponse.data && chatResponse.data.response) {
...
//     // Second response should be the greeting response
//     const greetingResponse = responses[1];
//     if (greetingResponse && greetingResponse.data && greetingResponse.data.response) {
//       return {
//         reply: greetingResponse.data.response,
//         timestamp: new Date(),
//       };
//     }
